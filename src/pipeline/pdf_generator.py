...
from reportlab.platypus import (SimpleDocTemplate, Paragraph, Spacer,
                                Table, TableStyle, ListItem, ListFlowable)
...
class PDFGenerator:
    ...
    def _header_footer(self, canvas, doc):
        ...
        canvas.drawString(72, doc.pagesize[1] - 50,
                          "IntelliCue — Confidential")
        # Footer: page numbers
    ...
    def generate_report(self, json_data):
        ...
        if 'overall_tone_summary' in json_data:
            elements.extend(self._process_overall_tone(
                json_data['overall_tone_summary']))

        # Process category insights
        if 'category_insights' in json_data:
            elements.extend(
                self._process_category_insights(
                    json_data['category_insights']))

        # Process key issues
        if 'key_issues' in json_data:
            elements.extend(
                self._process_key_issues(
                    json_data['key_issues']))

        # Process actionable next steps
        if 'actionable_next_steps' in json_data:
            elements.extend(
                self._process_action_items(
                    json_data['actionable_next_steps']))
    ...
    def _process_overall_tone(self, tone_data):
        ...
        elements.append(Paragraph("Overall Tone Summary",
        ...
        data = [
            ["Metric", "Value"],
            ["General Emotional Direction",
             tone_data.get('general_emotional_direction', 'N/A')],
            ["Positive Percentage",
             tone_data.get('positive_percentage', '0%')],
            ["Negative Percentage",
             tone_data.get('negative_percentage', '0%')],
            ["Neutral Percentage",
             tone_data.get('neutral_percentage', '0%')],
            ["Positive Count",
             str(tone_data.get('positive_count', 0))],
            ["Negative Count",
             str(tone_data.get('negative_count', 0))],
            ["Neutral Count",
             str(tone_data.get('neutral_count', 0))]
        ]
    ...
    def _process_category_insights(self, category_data):
        ...
        elements.append(Paragraph("Category Insights",
                                  self.styles['CustomHeading']))
        elements.append(Spacer(1, 12))
    ...
    def _process_action_items(self, action_items):
        ...
        elements.append(Paragraph("Actionable Next Steps",
                                  self.styles['CustomHeading']))
        elements.append(Spacer(1, 12))
    ...
    def _process_visuals(self, visuals_data):
        ...
        elements.append(Paragraph("Visual Summary",
                                  self.styles['CustomHeading']))
        elements.append(Spacer(1, 12))
